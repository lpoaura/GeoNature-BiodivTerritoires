...
FROM ref_geo.bib_areas_types
         LEFT OUTER JOIN ref_geo.l_areas ON ref_geo.l_areas.id_type = ref_geo.bib_areas_types.id_type);
    ...
    BEGIN
        FOR arrow in (select distinct cd_ref from taxonomie.t_redlist)
            LOOP
                ...
                FROM taxonomie.bib_redlist_categories,
                     taxonomie.bib_redlist_source,
                     taxonomie.t_redlist
                WHERE taxonomie.t_redlist.cd_ref = arrow.cd_ref
...
FROM taxonomie.bib_redlist_categories,
     taxonomie.bib_redlist_source,
     taxonomie.t_redlist
WHERE taxonomie.t_redlist.cd_ref = 79305
  AND taxonomie.bib_redlist_source.id_source = taxonomie.t_redlist.id_source
  AND taxonomie.bib_redlist_categories.code_category = taxonomie.t_redlist.category
ORDER BY taxonomie.bib_redlist_source.priority, taxonomie.bib_redlist_categories.priority_order
limit 1
