...
SELECT
  ...
  , count(DISTINCT taxref.cd_ref)
    FILTER (WHERE gn_biodivterritory.t_max_threatened_status.threatened = TRUE) AS count_threatened
  , count(DISTINCT synthese.id_synthese)                                        AS count_occtax
  , count(DISTINCT synthese.id_dataset)                                         AS count_dataset
  , count(DISTINCT synthese.date_min)                                           AS count_date
  , count(DISTINCT synthese.observers)                                          AS count_observer
...
--             JOIN observers ON observers.id_area = l_areas.id_area
            JOIN taxonomie.taxref ON synthese.cd_nom = taxref.cd_nom
            LEFT OUTER JOIN gn_biodivterritory.t_max_threatened_status
                            ON gn_biodivterritory.t_max_threatened_status.cd_nom = taxonomie.taxref.cd_ref
--             LEFT JOIN taxonomie.t_redlist ON taxref.cd_nom = t_redlist.cd_nom
--             JOIN taxonomie.bib_redlist_categories ON t_redlist.category = bib_redlist_categories.code_category
--             JOIN taxonomie.bib_redlist_source ON t_redlist.id_source = bib_redlist_source.id_source
    WHERE
      ...
      AND l_areas.id_type IN (SELECT id_type FROM gn_biodivterritory.l_areas_type_selection)
...
SELECT DISTINCT
  ...
  , CASE
        WHEN id_categorie_france IN ('CR', 'EN', 'VU') THEN
            TRUE
        ELSE
            FALSE
        END             AS threatened
...
SELECT
  ...
  , taxref.cd_ref
  , CASE
        WHEN categorie_lr_mondiale LIKE 'LR/%' THEN
            upper(
...
SELECT
  ...
  , taxref.cd_ref
  , CASE
        WHEN categorie_lr_europe LIKE 'LR/%' THEN
            upper(
            ...
            LOOP
                ...
                SELECT
                  ...
                  , bib_redlist_source.id_source
                    ...
                    ORDER BY
                        taxonomie.bib_redlist_source.priority, taxonomie.bib_redlist_categories.priority_order
                    LIMIT 1;
            END LOOP;
    END;
$$
