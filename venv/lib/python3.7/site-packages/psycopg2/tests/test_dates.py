...
class CommonDatetimeTestsMixin:
    ...
    def test_parse_incomplete_datetime(self):
        self.assertRaises(psycopg2.DataError, self.DATETIME, "2007", self.curs)
        self.assertRaises(psycopg2.DataError, self.DATETIME, "2007-01", self.curs)
        self.assertRaises(psycopg2.DataError, self.DATETIME, "2007-01-01 13", self.curs)
        self.assertRaises(
            psycopg2.DataError, self.DATETIME, "2007-01-01 13:30", self.curs
        )
...
class DatetimeTests(ConnectingTestCase, CommonDatetimeTestsMixin):
    ...
    def test_parse_bc_datetime(self):
        # datetime does not support BC dates
        self.assertRaises(
            ValueError, self.DATETIME, "00042-01-01 13:30:29 BC", self.curs
        )
    ...
    def test_parse_datetime_no_timezone(self):
        self.assertEqual(self.DATETIME("2007-01-01 13:30:29", self.curs).tzinfo, None)
        self.assertEqual(
    ...
    def test_adapt_date(self):
        ...
        value = self.execute("select (%s)::date::text", [date(2007, 1, 1)])
    ...
    def test_adapt_datetime(self):
        ...
        value = self.execute(
            "select (%s)::timestamp::text", [datetime(2007, 1, 1, 13, 30, 29)]
        )
    ...
    def test_adapt_timedelta(self):
        ...
        value = self.execute(
            "select extract(epoch from (%s)::interval)",
            [timedelta(days=42, seconds=45296, microseconds=123456)],
        )
...
class mxDateTimeTests(ConnectingTestCase, CommonDatetimeTestsMixin):
    ...
    def test_parse_datetime_timezone(self):
        ...
        self.assertEqual(expected, self.DATETIME("2007-01-01 13:30:29+01", self.curs))
        self.assertEqual(expected, self.DATETIME("2007-01-01 13:30:29-01", self.curs))
        self.assertEqual(
    ...
    def test_adapt_datetime(self):
        ...
        value = self.execute(
            "select (%s)::timestamp::text", [DateTime(2007, 1, 1, 13, 30, 29.123456)]
        )
    ...
    def test_adapt_bc_datetime(self):
        ...
        value = self.execute(
            "select (%s)::timestamp::text", [DateTime(-41, 1, 1, 13, 30, 29.123456)]
        )
        # microsecs for BC timestamps look not available in PG < 8.4
        # but more likely it's determined at compile time.
        self.assertTrue(
            value in ("0042-01-01 13:30:29.123456 BC", "0042-01-01 13:30:29 BC"), value
        )
    ...
    def test_adapt_timedelta(self):
        ...
        value = self.execute(
            "select extract(epoch from (%s)::interval)",
            [DateTimeDeltaFrom(days=42, seconds=45296.123456)],
        )
        seconds = math.floor(value)
        self.assertEqual(seconds, 3674096)
        self.assertEqual(int(round((value - seconds) * 1000000)), 123456)
...
class FromTicksTestCase(unittest.TestCase):
    ...
    def test_timestamp_value_error_sec_59_99(self):
        ...
        self.assertEqual(
            s.adapted,
            datetime(
                2010, 5, 6, 14, 11, 59, 999920, tzinfo=FixedOffsetTimezone(-5 * 60)
            ),
        )
    ...
    def test_time_value_error_sec_59_99(self):
        ...
        self.assertEqual(s.adapted.replace(hour=0), time(0, 11, 59, 999920))
...
class FixedOffsetTimezoneTests(unittest.TestCase):
    ...
    def test_repr_with_positive_offset(self):
        ...
        self.assertEqual(
            repr(tzinfo), "psycopg2.tz.FixedOffsetTimezone(offset=300, name=None)"
        )
    ...
    def test_repr_with_name(self):
        ...
        self.assertEqual(
            repr(tzinfo), "psycopg2.tz.FixedOffsetTimezone(offset=0, name='FOO')"
        )

    def test_instance_caching(self):
        self.assertTrue(
            FixedOffsetTimezone(name="FOO") is FixedOffsetTimezone(name="FOO")
        )
        self.assertTrue(FixedOffsetTimezone(7 * 60) is FixedOffsetTimezone(7 * 60))
        self.assertTrue(
            FixedOffsetTimezone(-9 * 60, "FOO") is FixedOffsetTimezone(-9 * 60, "FOO")
        )
        self.assertTrue(
            FixedOffsetTimezone(9 * 60) is not FixedOffsetTimezone(9 * 60, "FOO")
        )
        self.assertTrue(
            FixedOffsetTimezone(name="FOO") is not FixedOffsetTimezone(9 * 60, "FOO")
        )
