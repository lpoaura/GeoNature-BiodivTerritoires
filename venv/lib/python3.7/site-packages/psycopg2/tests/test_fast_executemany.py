...
class TestExecuteBatch(FastExecuteTestMixin, testutils.ConnectingTestCase):
    def test_empty(self):
        ...
        psycopg2.extras.execute_batch(
            cur, "insert into testfast (id, val) values (%s, %s)", []
        )
...
class TestExecuteValues(FastExecuteTestMixin, testutils.ConnectingTestCase):
    def test_empty(self):
        ...
        psycopg2.extras.execute_values(
            cur, "insert into testfast (id, val) values %s", []
        )
        cur.execute("select * from testfast order by id")
        self.assertEqual(cur.fetchall(), [])
    ...
    def test_one(self):
        cur = self.conn.cursor()
        psycopg2.extras.execute_values(
            cur, "insert into testfast (id, val) values %s", iter([(1, 10)])
        )
        cur.execute("select id, val from testfast order by id")
        self.assertEqual(cur.fetchall(), [(1, 10)])
    ...
    def test_unicode(self):
        ...
        # unicode in data
        psycopg2.extras.execute_values(
            cur, "insert into testfast (id, data) values %s", [(2, snowman)]
        )
        cur.execute("select id, data from testfast where id = 2")
        self.assertEqual(cur.fetchone(), (2, snowman))
    ...
    def test_invalid_sql(self):
        ...
        self.assertRaises(
            ValueError, psycopg2.extras.execute_values, cur, "insert %f %s", []
        )
...
testutils.decorate_all_tests(TestExecuteValues, testutils.skip_before_postgres(8, 2))
