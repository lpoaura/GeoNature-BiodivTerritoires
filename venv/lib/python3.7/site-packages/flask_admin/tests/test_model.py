...
def test_column_filters():
    ...
    eq_(
        [(f["index"], f["operation"]) for f in view._filter_groups[u"col1"]],
        [(0, "test")],
    )
    eq_(
        [(f["index"], f["operation"]) for f in view._filter_groups[u"col2"]],
        [(1, "test")],
    )
...
def test_export_csv():
    ...
    view = MockModelView(
        Model, view_data, can_export=True, column_list=["col1", "col2"], endpoint="utf8"
    )
    admin.add_view(view)
