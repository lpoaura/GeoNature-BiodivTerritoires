...
def geom_factory(g, parent=None):
    ...
    mod = __import__("shapely.geometry", globals(), locals(), [geom_type],)
...
def geom_from_wkt(data):
    warn(
        "`geom_from_wkt` is deprecated. Use `geos.wkt_reader.read(data)`.",
        DeprecationWarning,
    )
    if sys.version_info[0] >= 3:
...
def geom_to_wkt(ob):
    warn(
        "`geom_to_wkt` is deprecated. Use `geos.wkt_writer.write(ob)`.",
        DeprecationWarning,
    )
...
def geom_from_wkb(data):
    warn(
        "`geom_from_wkb` is deprecated. Use `geos.wkb_reader.read(data)`.",
        DeprecationWarning,
    )
...
def geom_to_wkb(ob):
    warn(
        "`geom_to_wkb` is deprecated. Use `geos.wkb_writer.write(ob)`.",
        DeprecationWarning,
    )
    if ob is None or ob._geom is None:
        raise ValueError("Null geometry supports no operations")
...
class BaseGeometry(object):
    ...
    def to_wkb(self):
        warn("`to_wkb` is deprecated. Use the `wkb` property.", DeprecationWarning)
        return geom_to_wkb(self)

    def to_wkt(self):
        warn("`to_wkt` is deprecated. Use the `wkt` property.", DeprecationWarning)
    ...
    def buffer(
        ...
        if "buffer_with_style" not in self.impl:
            raise NotImplementedError(
                "Styled buffering not available for " "GEOS versions < 3.2."
            )
...
class BaseMultipartGeometry(BaseGeometry):
    ...
    def svg(self, scale_factor=1.0, color=None):
        ...
        return "<g>" + "".join(p.svg(scale_factor, color) for p in self) + "</g>"
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, integer_types):
            ...
            if type(self) == HeterogeneousGeometrySequence:
                raise TypeError("Heterogenous geometry collections are not sliceable")
            res = []
