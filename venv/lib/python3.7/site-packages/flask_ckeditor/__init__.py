...
class _CKEditor(object):
    ...
    def load(custom_url=None, pkg_type=None, serve_local=None, version="4.9.2"):
        ...
        if pkg_type not in ["basic", "standard", "full"]:
            warnings.warn(
                "The provided pkg_type string was invalid, "
    ...
    def config(name="ckeditor", custom_config="", **kwargs):
        ...
        file_browser = kwargs.get(
            "file_browser", current_app.config["CKEDITOR_FILE_BROWSER"]
        )
        ...
        enable_codesnippet = (
            kwargs.get("enable_codesnippet", wrong_key_arg)
            or current_app.config["CKEDITOR_ENABLE_CODESNIPPET"]
        )
        ...
        enable_csrf = kwargs.get(
            "enable_csrf", current_app.config["CKEDITOR_ENABLE_CSRF"]
        )
...
<script type="text/javascript">
    ...
    def create(name="ckeditor", value=""):
        ...
        """
        return Markup(
            '<textarea class="ckeditor" name="%s">%s</textarea>' % (name, value)
        )
