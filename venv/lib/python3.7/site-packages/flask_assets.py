...
class FlaskResolver(Resolver):
    ...
    def use_webassets_system_for_output(self, ctx):
        return (
            ctx.config.get("directory") is not None or ctx.config.get("url") is not None
        )
...
try:
    ...
    class FlaskArgparseInterface(GenericArgparseImplementation):
        ...
        def _construct_parser(self, *a, **kw):
            super(FlaskArgparseInterface, self)._construct_parser(*a, **kw)
            ...
            if env is not None:
                if ns.parse_templates:
                    ...
                    env.add(
                        *[
                            b
                            for b in self.load_from_templates(env, ns.jinja_extension)
        ...
        def load_from_templates(self, env, jinja_extension):
            ...
            return Jinja2Loader(
                env, template_dirs, [jinja2_env], jinja_ext=jinja_extension
            ).load_bundles()
    ...
    class ManageAssets(script.Command):
        ...
        def __init__(self, assets_env=None, impl=FlaskArgparseInterface, log=None):
            self.env = assets_env
