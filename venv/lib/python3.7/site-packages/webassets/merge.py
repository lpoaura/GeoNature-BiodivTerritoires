...
class FilterTool(object):
    ...
    def _wrap_cache(self, key, func):
        ...
        if self.cache:
            log.debug(
                "Storing result in cache with key %s", key,
            )
    ...
    def apply(self, hunk, filters, type, kwargs=None):
        ...
        log.debug(
            'Need to run method "%s" of filters (%s) on hunk %s with ' "kwargs=%s",
            type,
            filters,
            hunk,
            kwargs,
        ...
        if not filters:  # Short-circuit
            log.debug(
                'No filters have "%s" methods, returning hunk ' "unchanged" % (type,)
            )
            return hunk
        ...
        def func():
            ...
            for filter in filters:
                log.debug(
                    'Running method "%s" of  %s with kwargs=%s',
                    type,
                    filter,
                    kwargs_final,
                )
    ...
    def apply_func(self, filters, type, args, kwargs=None, cache_key=None):
        ...
        def func():
            ...
            out = StringIO(u"")  # For 2.x, StringIO().getvalue() returns str
            log.debug(
                'Running method "%s" of %s with args=%s, kwargs=%s',
                type,
                filter,
                args,
                kwargs,
...
def select_filters(filters, level):
    ...
    return [
        f
        for f in filters
        if f.max_debug_level is None or cmp_debug_levels(level, f.max_debug_level) <= 0
    ]
