...
class Bundle(object):
    ...
    def __init__(self, *contents, **options):
        ...
        if options:
            raise TypeError(
                "got unexpected keyword argument '%s'" % list(options.keys())[0]
            )
    ...
    def id(self):
        ...
        """
        return hash_func(
            (tuple(self.contents), self.output, tuple(self.filters), bool(self.debug))
        )
    ...
    def _merge_and_apply(
        ...
        filtertool = FilterTool(
            ...
            kwargs={"output": output[0], "output_path": output[1]},
        )
    ...
    def _build(
        self, ctx, extra_filters=None, force=None, output=None, disable_cache=None
    ):
        ...
        if force:
            update_needed = True
        elif not has_placeholder(self.output) and not path.exists(
            self.resolve_output(ctx, self.output)
        ):
            update_needed = True
        else:
            update_needed = (
                ctx.updater.needs_rebuild(self, ctx) if ctx.updater else True
            )
        ...
        if self.is_container:
            for bundle, _ in self.resolve_contents(ctx):
                if bundle.is_container:
                    for child, child_filters, new_ctx in bundle.iterbuild(
                        wrap(ctx, bundle)
                    ):
                        yield (
                            child,
    ...
    def _urls(self, ctx, extra_filters, *args, **kwargs):
        ...
        if supposed_to_merge and (self.filters or self.output):
            ...
            if ctx.auto_build:
                self._build(
                    ctx, extra_filters=extra_filters, force=False, *args, **kwargs
                )
...
def pull_external(ctx, filename):
    ...
    rel_path = path.join(
        "webassets-external", "%s_%s" % (hashed_filename, path.basename(filename))
    )
...
has_files = lambda bundle: any(
    [c for c in bundle.contents if not isinstance(c, Bundle)]
)
