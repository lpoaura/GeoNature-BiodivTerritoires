...
class Filter(object):
    ...
    def get_config(
        self, setting=False, env=None, require=True, what="dependency", type=None
    ):
        ...
        for attribute, (_, configvar, type) in self._options.items():
            ...
            if getattr(self, attribute) is None:
                ...
                setattr(
                    self,
                    attribute,
                    self.get_config(setting=configvar, require=False, type=type),
                )
...
class ExternalTool(six.with_metaclass(ExternalToolMetaclass, Filter)):
    ...
    def subprocess(cls, argv, out, data=None, cwd=None):
        ...
        if hasattr(str, "format"):  # Support Python 2.5 without the feature
            argv = list(
                map(
                    lambda item: item.format(input=input_file, output=output_file), argv
                )
            )
...
def load_builtin_filters():
    ...
    for module_name in module_names:
        ...
        try:
            ...
            warnings.warn(
                "Error while loading builtin filter "
