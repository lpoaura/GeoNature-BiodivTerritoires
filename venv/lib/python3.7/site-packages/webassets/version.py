    ...
    def determine_version(self, bundle, ctx, hunk=None):
        ...
        try:
            ...
            raise VersionIndeterminableError(
                "source files are missing and output target has a " "placeholder"
            )
...
class Manifest(
    six.with_metaclass(
        RegistryMetaclass(clazz=lambda: Manifest, desc="a manifest implementation")
    )
):
    ...
    def _check(self, ctx):
        if not ctx.cache:
            raise EnvironmentError(
                "You are using the cache manifest, but have not " "enabled the cache."
            )
