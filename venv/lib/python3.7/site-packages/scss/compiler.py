    ...
    def __init__(
        ...
        for extension in extensions:
            if isinstance(extension, Extension):
                ...
                raise TypeError(
                    "Expected an Extension or Namespace, got: {0!r}".format(extension)
                )
...
class Compilation(object):
    ...
    def __init__(self, compiler):
        ...
        self.root_namespace = Namespace.derive_from(
            *(ext.namespace for ext in compiler.extensions if ext.namespace)
        )
    ...
    def run(self):
        ...
        if total_selectors >= 4096:
            log.error(
                "Maximum number of supported selectors in Internet Explorer (4095) exceeded!"
            )
            ...
            if block.is_atrule:
                ...
                try:
                    ...
                    if block.unparsed_contents is None:
                        ...
                        scope is None
                    ):  # needs to have no scope to crawl down the nested rules
    ...
    def _at_option(self, calculator, rule, scope, block):
        ...
        for option in block.argument.split(","):
            ...
            if key in ("short_colors", "reverse_colors"):
                warn_deprecated(
                    rule, "The '{0}' @option no longer has any effect.".format(key),
                )
                return
            elif key == "style":
                try:
                ...
                if not isinstance(value, bool):
                    raise SassError(
                        "The '{0}' @option requires a bool, not {1!r}".format(
                            key, value
                        )
                    )
            else:
    ...
    def _get_funct_def(self, rule, calculator, argument):
        ...
        if lpar:
            if not argstr.endswith(")"):
                raise SyntaxError(
                    "Expected ')', found end of line for %s (%s)"
    ...
    def _populate_namespace_from_call(
        self, name, callee_namespace, mixin, args, kwargs
    ):
        # Mutation protection
        args = list(args)
        ...
        if callee_argspec.slurp:
            ...
            callee_namespace.set_variable(
                callee_argspec.slurp.name, Arglist(args, sass_kwargs)
            )
            args = []
            kwargs = {}
    ...
    def _at_include(self, calculator, rule, scope, block):
        ...
        if caller_argspec.inject and callee_argspec.inject:
            ...
            callee_namespace = Namespace.derive_from(
                caller_namespace, pristine_callee_namespace
            )
        else:
            callee_namespace = pristine_callee_namespace.derive()

        self._populate_namespace_from_call(
            "Mixin {0}".format(funct), callee_namespace, mixin, args, kwargs
        )
    ...
    def _at_import(self, calculator, rule, scope, block):
        ...
        for sass_path in sass_paths:
            ...
            if (
                # If it's a url()
                isinstance(sass_path, Url)
                or
                # If it's not a string (including `"foo" screen`, a List)
                not isinstance(sass_path, String)
                or
                # If the filename begins with an http protocol
                sass_path.value.startswith(("http://", "https://"))
                or
    ...
    def _get_properties(self, rule, scope, block):
        ...
        if is_var or prop.startswith("$") and raw_value is not None:
            ...
            if is_default and is_defined:
                ...
                rule.namespace.set_variable(_prop, value, local_only=not is_global)
        else:
            ...
            for i, header in reversed(list(enumerate(new_ancestry))):
                if header.is_selector:
                    ...
                    new_ancestry[i] = BlockAtRuleHeader(
                        "@media", "%s and %s" % (header.argument, block.argument)
                    )
                    break
                else:
            ...
            for selector in rule.extends_selectors:
                ...
                if not extendable_selectors:
                    ...
                    warn_deprecated(
                        ...
                        "will be fatal in 2.0, unless !optional is specified!".format(
                            selector.render()
                        ),
                    )
                    continue
                ...
                for extendable_selector in extendable_selectors:
                    ...
                    for parent_rule in parent_rules:
                        if parent_rule is rule:
                            ...
                            continue
                        known_parents.append((parent_rule, extendable_selector))
                ...
                for parent_rule, extendable_selector in known_parents:
                    ...
                    for rule_selector in rule.selectors:
                        more_parent_selectors.extend(
                            extendable_selector.substitute(selector, rule_selector)
                        )
                    ...
                    parent_rule.ancestry = parent_rule.ancestry.with_more_selectors(
                        more_parent_selectors
                    )
    ...
    def create_css(self, rules):
        ...
        if style == "legacy":
            sc, sp, tb, nst, srnl, nl, rnl, lnl, dbg = (
                True,
                " ",
                "  ",
                False,
    ...
    def _create_css(
        ...
        for rule in rules:
            ...
            first_mismatch = 0
            for i, (old_header, new_header) in enumerate(
                zip(prev_ancestry_headers, ancestry.headers)
            ):
            ...
            if (
                not sc
                and dangling_property
                and first_mismatch >= len(prev_ancestry_headers)
            ):
                result += ";"
            ...
            for i in range(first_mismatch, ancestry_len):
                ...
                if debug_info:
                    ...
                    def _print_debug_info(filename, lineno):
                        if debug_info == "comments":
                            ...
                            result = (
                                tb * (i + nesting)
                                + "@media -sass-debug-info{filename{font-family:file\:\/\/%s}line{font-family:\\00003%s}}"
                                % (filename, lineno)
                                + nl
                            )
                        return result
                ...
                if header.is_selector:
                    ...
                    if nl:
                        header_string = (nl + tb * (i + nesting)).join(
                            self._textwrap(header_string)
                        )
                else:
                    header_string = header.render()
            ...
            if not skip_selectors:
                result += self._print_properties(
                    rule.properties, sc, sp, tb * (ancestry_len + nesting), nl, lnl
                )
