    ...
    def is_superset_of(self, other, soft_combinator=False):
        ...
        if soft_combinator and self.combinator == " ":
            ...
            combinator_superset = (
                ...
                or (self.combinator == " " and other.combinator == ">")
                or (self.combinator == "~" and other.combinator == "+")
            )

        return combinator_superset and set(self.tokens) <= set(other.tokens)
    ...
    def replace_parent(self, parent_simples):
                ...
                if token == "self":
                    warn(
                        FutureWarning(
                            ...
                            "removed in 2.0.  Use & instead.  ({0!r})".format(self)
                        )
                    )
            else:
        ...
        if not _is_combinator_subset_of(self.combinator, root.combinator):
            raise ValueError(
                ...
                "combinators {2!r} and {3!r} conflict!".format(
    ...
    def merge_into(self, other):
        ...
        if self.combinator == " " or self.combinator == other.combinator:
            ...
            raise ValueError(
                ...
                "{0!r} and {1!r}".format(self, other)
            )
        return type(self)(combinator, new_tokens)

    def difference(self, other):
        new_tokens = tuple(
            token for token in self.tokens if token not in set(other.tokens)
        )
...
class Selector(object):
    ...
    def parse_many(cls, selector):
        ...
        pending = dict(simples=[], combinator=" ", tokens=[],)
    ...
    def substitute(self, target, replacement):
        ...
        cls = type(self)
        return [cls(before + focal_nodes + p_after) for before in befores]
...
def _merge_selectors(left, right):
    ...
    for left_next, right_next, merged in lcs:
        ret = _weave_conflicting_selectors(
            ret, left[left_last:left_next], right[right_last:right_next], (merged,)
        )

        left_last = left_next + 1
        right_last = right_next + 1

    ret = _weave_conflicting_selectors(ret, left[left_last:], right[right_last:])
    ...
    for ai in range(-1, len(a)):
        for bi in range(-1, len(b)):
            if ai == -1 or bi == -1:
                ...
                l = max(prefix_lcs_length[ai, bi - 1], prefix_lcs_length[ai - 1, bi])
