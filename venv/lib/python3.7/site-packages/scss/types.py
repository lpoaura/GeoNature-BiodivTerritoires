...
class Number(Value):
    ...
    def _compare(self, other, op, soft_fail=False):
        ...
        if self.is_unitless or other.is_unitless or self.value == 0 or other.value == 0:
            left = self
            right = other
        else:
            ...
            if (
                ...
                or left.unit_denom != right.unit_denom
            ):
                if soft_fail:
    ...
    def _add_sub(self, other, op):
        ...
        return Number(
            new_amount, unit_numer=self.unit_numer, unit_denom=self.unit_denom
        )
...
class List(Value):
    ...
    def __init__(self, iterable, separator=None, use_comma=None, literal=False):
        ...
        if not isinstance(iterable, Iterable) or isinstance(iterable, six.string_types):
    ...
    def __repr__(self):
        return "<{0} {1}>".format(
            type(self).__name__, self.delimiter().join(repr(item) for item in self),
        )
    ...
    def render(self, compress=False):
        ...
        return delim.join(item.render(compress=compress) for item in value)
    ...
    def __add__(self, other):
        if isinstance(other, List):
            ...
            return List(
                [item + max_list[i] for i, item in enumerate(min_list)],
                use_comma=self.use_comma,
            )
    ...
    def _operate(self, other, op):
        if isinstance(other, Number):
            ...
            if self.alpha != other.alpha:
                raise ValueError(
                    "Alpha channels must match between %r and %r" % (self, other)
                )
        ...
        new_rgb = [
            min(255.0, max(0.0, op(left, right)))
            # for from_rgb
            / 255.0
...
class String(Value):
        ...
        if isinstance(value, six.binary_type):
            warn(
                FutureWarning(
                    ...
                    "-- this will no longer be supported in pyScss 2.0".format(value)
                )
            )
    ...
    def __repr__(self):
        if self.quotes:
            ...
            quotes = ""
        return "<{0}{1} {2!r}>".format(type(self).__name__, quotes, self.value)
    ...
    def __add__(self, other):
        ...
        return String(self.value + other_value, quotes='"' if self.quotes else None)
...
class Function(String):
    ...
    def render(self, compress=False):
        return "{0}({1})".format(
            self.function_name, super(Function, self).render(compress),
        )

    def render_interpolated(self, compress=False):
        return "{0}({1})".format(
            self.function_name, super(Function, self).render_interpolated(compress),
        )
...
class Url(Function):
    ...
    def render_interpolated(self, compress=False):
        ...
        # is valid CSS.
        inside = self.bad_identifier_rx.sub(self._escape_character, self.value)
...
def expect_type(value, types, unit=any):
    ...
    if unit is not any and isinstance(value, Number):
        ...
        elif unit == "%" and not (value.is_unitless or value.is_simple_unit("%")):
            raise ValueError(
