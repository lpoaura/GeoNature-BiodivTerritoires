...
class SourceFile(object):
    ...
    def __init__(self, origin, relpath, contents, encoding=None, is_sass=None):
        ...
        if not isinstance(contents, six.text_type):
            raise TypeError(
                "Expected text for 'contents', got {0}".format(type(contents))
            )

        if origin and ".." in relpath.parts:
            raise ValueError("relpath cannot contain ..: {0!r}".format(relpath))
    ...
    def from_string(cls, string, relpath=None, encoding=None, is_sass=None):
        ...
        if relpath is None:
            ...
            relpath = repr(
                "string:{0}:{1}".format(m.hexdigest()[:16], text_contents[:100])
            )

        return cls(origin, relpath, text_contents, encoding=encoding, is_sass=is_sass,)
