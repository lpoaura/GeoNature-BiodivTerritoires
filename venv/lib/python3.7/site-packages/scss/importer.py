...
class FilesystemImporter(Importer):
    ...
    def __repr__(self):
        return "<{0}({1!r})>".format(type(self).__name__, text_type(self.root))
    ...
    def find(self, path, relative_to=".", **kwargs):
        ...
        if len(exists) == 0:
            ...
            return Module.from_filename(
                self.root / candpath, syntax=syntax, importer=self
            )
        ...
        raise SassImportError(
            ...
            "(you should rename or delete all but one of these):\n  {1}".format(
                text_type(path),
                "\n  ".join(
                    text_type(self.root / candpath) for candpath, syntax in exists
                ),
            )
        )
