...
BUILTIN_FUNCTIONS = frozenset(
    [
        ...
        "mask",  # ???
        "from",
        "to",  # Very old WebKit gradient syntax
    ]
)
...
def determine_encoding(buf):
    ...
    if buf.startswith(charset_start):
        ...
        if encoded_charset.decode(encoding) != encoded_charset.decode(bom_encoding):
            raise UnicodeError(
                "@charset {0} is incompatible with detected encoding {1}".format(
...
CSS2_PSEUDO_ELEMENTS = frozenset((":after", ":before", ":first-line", ":first-letter",))
...
def _unescape_one(match):
    if match.group(1) is not None:
        try:
            ...
            return (r"\U%08x" % int(match.group(1), 16)).decode("unicode-escape")
