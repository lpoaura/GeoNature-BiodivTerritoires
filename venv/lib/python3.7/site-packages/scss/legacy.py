...
class Scss(object):
    ...
    def compile(
        ...
        root_namespace = Namespace(variables=self.scss_vars, functions=self._library,)
        ...
        if self._search_paths is not None:
            assert not isinstance(
                self._search_paths, six.string_types
        ...
        if source_files is not None:
            ...
            source = SourceFile.from_string(
                scss_string, relpath=filename, is_sass=is_sass,
            )
            compilation.add_source(source)
            ...
            with open(scss_file, "rb") as f:
                source = SourceFile.from_file(
                    f, relpath=filename or scss_file, is_sass=is_sass,
                )
            compilation.add_source(source)
        ...
        self.source_files = list(
            SourceFileTuple(*os.path.split(s.path))
            for s in compilation.source_index.values()
        )
        return compiled
