    ...
    def locate_blocks(codestr):
        ...
        for m in _blocks_re.finditer(codestr):
            ...
            if c == "\\":
                ...
                if c == "{":  # block begins:
                    ...
                    if depth <= 0:
                        raise SyntaxError(
                            "Unexpected closing brace on line {0}".format(lineno)
                        )
                    else:
                        depth -= 1
                        if depth == 0:
        ...
        if depth > 0:
            if not skip:
                ...
                raise SyntaxError(
                    "Couldn't parse block starting on line {0}: {1}".format(
                        lineno, error
                    )
                )
...
class Parser(object):
    ...
    def _scan(self, type):
        ...
        if tok[2] != type:
            raise SyntaxError(
                "SyntaxError[@ char %s: %s]" % (repr(tok[0]), "Trying to find " + type)
            )
        self._pos += 1
        return tok[3]
    ...
    class Scanner(object):
            ...
            while True:
                ...
                for tok, regex in self.patterns:
                    ...
                    if m:
                        ...
                        if DEBUG:
                            print(
                                "Match OK! %s: %s at pos %d"
                                % (repr(tok), repr(regex.pattern), self.pos)
                            )
                        break
        ...
        def token(self, i, restrict=None):
            ...
            if i < tokens_len:
                if (
                    restrict
                    and self.restrictions[i]
                    and restrict > self.restrictions[i]
                ):
