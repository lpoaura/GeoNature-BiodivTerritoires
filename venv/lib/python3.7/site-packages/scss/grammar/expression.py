...
class SassExpressionScanner(Scanner):
    ...
    _patterns = [
        ...
        (
            "LITERAL_FUNCTION",
            "(-moz-calc|-webkit-calc|calc|expression|progid:[\\w.]+)(?=[(])",
        ),
...
class SassExpression(Parser):
    ...
    def expr_map_or_list(self):
        ...
        if _token_ == '":"':
            ...
            while self._peek(self.expr_map_or_list_rsts_) == '","':
                ...
                map_item = None, None
                if (
                    self._peek(self.expr_map_or_list_rsts__)
    ...
    def atom(self):
        ...
        if _token_ == "LPAR":
            ...
            RPAR = self._scan("RPAR")
            return Interpolation.maybe(
                interpolated_function, type=Function, function_name=LITERAL_FUNCTION
            )
        elif _token_ == "FNCT":
    ...
    def interpolated_function_parens(self):
        ...
        while self._peek(self.interpolated_function_parens_rsts) == "LPAR":
            ...
            parts = (
                parts[:-1]
                + [parts[-1] + LPAR + interpolated_function[0]]
                + interpolated_function[1:]
            )
            RPAR = self._scan("RPAR")
            INTERP_NO_PARENS = self._scan("INTERP_NO_PARENS")
    ...
    expr_map_or_list_rsts__ = frozenset(
        [
            "LPAR",
            "RPAR",
            "BANG_IMPORTANT",
    ...
    interpolated_bare_url_rsts_ = frozenset(
        ["RPAR", "INTERP_START", "BAREURL", "SPACE"]
    )
    ...
    interpolated_bareword_rsts = frozenset(
        [
            "LPAR",
            "DOUBLE_QUOTE",
            "SUB",
    ...
    interpolated_bareword_rsts_ = frozenset(
        [
            "LPAR",
            "DOUBLE_QUOTE",
            "SUB",
    ...
    interpolated_url_rsts = frozenset(
        [
            "LPAR",
            "DOUBLE_QUOTE",
            "BANG_IMPORTANT",
    ...
    interpolated_bareword_rsts__ = frozenset(
        [
            "LPAR",
            "DOUBLE_QUOTE",
            "SUB",
    ...
    argspec_items_rsts__ = frozenset(
        [
            "KWVAR",
            "LPAR",
            "DOUBLE_QUOTE",
