...
def main():
    ...
    parser.add_option(
        "-i", "--interactive", action="store_true", help="Run an interactive Scss shell"
    )
    parser.add_option(
        "-w",
        "--watch",
    ...
    parser.add_option(
        "--time", action="store_true", help="Ignored, will be removed in 2.0"
    )
    parser.add_option(
        "--debug-info",
        action="store_true",
    ...
    parser.add_option(
        "--sass", action="store_true", dest="is_sass", default=None, help="Sass mode"
    )
...
def do_build(options, args):
    ...
    css = Scss(
        scss_opts={"style": options.style, "debug_info": options.debug_info,},
    ...
    for path in args:
        if path == "-":
            source = SourceFile.from_file(
                sys.stdin, relpath="<stdin>", is_sass=options.is_sass
            )
        else:
...
def watch_sources(options):
    ...
    if options.output and not os.path.isdir(options.output):
        sys.stderr.write(
            "watch file output directory is invalid: '%s'" % (options.output)
        )
        sys.exit(2)
    ...
    class ScssEventHandler(PatternMatchingEventHandler):
        ...
        def is_valid(self, path):
            return (
                os.path.isfile(path)
                and (path.endswith(".scss") or path.endswith(".sass"))
            ...
            if not s:
                ...
                if m:
                    ...
                    name = name and name.strip().rstrip(
                        "s"
                    )  # remove last 's' as in functions
                    code = code and code.strip()
                    ns = self.namespace
                    if not name:
                        yield pformat(
                            list(sorted(["vars", "options", "mixins", "functions"]))
                        )
                        ...
                        if code == "*":
                            ...
                            variables = dict(
                                (k, v) for k, v in variables.items() if code in k
                            )
                        else:
                            variables = dict(
                                (k, v)
                        ...
                        if code == "*":
                            ...
                            funcs = dict(
                                (k, v) for k, v in funcs.items() if code in k[0]
                            )
                        else:
                            pass
