...
class BinaryOp(Expression):
    ...
    def evaluate(self, calculator, divide=False):
        ...
        # shebang as literal.
        if any(
            isinstance(operand, Interpolation) and operand.parts[0] == ""
...
class CallOp(Expression):
    ...
    def evaluate(self, calculator, divide=False):
        ...
        kwargs = dict(
            (key.lstrip("$").replace("-", "_"), value) for key, value in kwargs.items()
        )
        ...
        if kwargs:
            raise TypeError(
                "The CSS function %s doesn't support keyword arguments." % (func_name,)
            )

        # TODO another candidate for a "function call" sass type
        rendered_args = [arg.render() for arg in args]

        return String("%s(%s)" % (func_name, ", ".join(rendered_args)), quotes=None)
...
class Literal(Expression):
    ...
    def evaluate(self, calculator, divide=False):
        if isinstance(self.value, Undefined) and calculator.undefined_variables_fatal:
        ...
        try:
            ...
            if isinstance(value, six.string_types):
                log.warn(
                    ...
                    "but found a string expression: {1!r}".format(self.name, value)
                )
...
class MapLiteral(Expression):
    ...
    def evaluate(self, calculator, divide=False):
        ...
        for key, value in self.pairs:
            scss_pairs.append((key.evaluate(calculator), value.evaluate(calculator),))
