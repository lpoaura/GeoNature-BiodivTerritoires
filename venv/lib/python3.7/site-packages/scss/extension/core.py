...
class CoreExtension(Extension):
    ...
    def handle_import(self, name, compilation, rule):
        ...
        search_path.extend(
            (origin, relative_to) for origin in compilation.compiler.search_path
        )
...
def mix(color1, color2, weight=Number(50, "%")):
    ...
    channels = [
        ch1 * w1 + ch2 * w2 for (ch1, ch2) in zip(color1.rgba[:3], color2.rgba[:3])
    ]
...
def ie_hex_str(color):
    c = Color(color).value
    return String(
        "#{3:02X}{0:02X}{1:02X}{2:02X}".format(
            int(round(c[0])),
            int(round(c[1])),
            int(round(c[2])),
            int(round(c[3] * 255)),
...
@ns.declare
def scale_color(
    color, red=None, green=None, blue=None, saturation=None, lightness=None, alpha=None
):
    do_rgb = red or green or blue
...
@ns.declare
def zip_(*lists):
    return List([List(zipped) for zipped in zip(*lists)], use_comma=True)
...
@ns.declare
def map_keys(map):
    return List([k for (k, v) in map.to_pairs()], use_comma=True)


@ns.declare
def map_values(map):
    return List([v for (k, v) in map.to_pairs()], use_comma=True)
...
def comparable(number1, number2):
    ...
    return Boolean(
        left.unit_numer == right.unit_numer and left.unit_denom == right.unit_denom
    )
