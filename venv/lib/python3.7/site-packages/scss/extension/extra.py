...
def _image_noise(
    ...
    if not color:
        color = [(0, 0, 0, 0)]
    elif not isinstance(color, (tuple, list)) or not isinstance(
        color[0], (tuple, list)
    ):
        color = [color]
    ...
    for pos, col in pixels.items():
        ...
        if ca:
            pixdata[pos] = tuple(
                int(round(c)) for c in (col[0] / ca, col[1] / ca, col[2] / ca, ca * 255)
            )
...
def _image_brushed(
    ...
    loops = max(
        map(len, (density, intensity, color, opacity, monochrome, direction, spread))
    )
    for l in range(loops):
        _density = density[l % len(density)]
        _intensity = intensity[l % len(intensity)]
...
def background_noise(
    ...
    if not inline:
        ...
        asset_path = os.path.join(
            config.ASSETS_ROOT or os.path.join(config.STATIC_ROOT, "assets"), asset_file
        )
        try:
...
def grid_image(
    ...
    _right_gutter = (
        int(right_gutter) if right_gutter >= 1 else int(right_gutter * 1000.0)
    )
    if _height <= 0 or _width <= 0 or _left_gutter <= 0 or _right_gutter <= 0:
        raise ValueError
    _full_width = _left_gutter + _width + _right_gutter
    new_image = Image.new(
        mode="RGBA", size=(_full_width * int(columns), _height), color=background_color
    )
    draw = ImageDraw.Draw(new_image)
    for i in range(int(columns)):
        draw.rectangle(
            (
                i * _full_width + _left_gutter,
                0,
                i * _full_width + _left_gutter + _width - 1,
                _height - 1,
            ),
            fill=grid_color,
        )
...
def image_color(color, width=1, height=1):
    ...
    if w <= 0 or h <= 0:
        raise ValueError
    new_image = Image.new(
        mode="RGB" if color.alpha == 1 else "RGBA", size=(w, h), color=color.rgba255,
    )
    output = six.BytesIO()
    new_image.save(output, format="PNG")
    contents = output.getvalue()
