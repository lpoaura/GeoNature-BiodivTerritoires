...
def ttfautohint(ttf):
    try:
        ...
        if e.errno in (errno.EACCES, errno.ENOENT):
            warnings.warn(
                "Could not autohint ttf font: The executable %s could not be run: %s"
                % (TTFAUTOHINT_EXECUTABLE, e)
            )
            return None
        else:
            raise e
...
def ttf2eot(ttf):
    try:
        proc = subprocess.Popen(
            [TTF2EOT_EXECUTABLE], stdin=subprocess.PIPE, stdout=subprocess.PIPE,
        )
    except OSError as e:
        if e.errno in (errno.EACCES, errno.ENOENT):
            warnings.warn(
                "Could not generate eot font: The executable %s could not be run: %s"
                % (TTF2EOT_EXECUTABLE, e)
            )
            return None
        else:
            raise e
...
def font_sheet(g, **kwargs):
    ...
    if files is not None:
        ...
        asset_paths = dict(
            (type_, os.path.join(ASSETS_ROOT, asset_file))
        ...
        inline_assets = {}
        if (
            all(os.path.exists(asset_path) for asset_path in asset_paths.values())
            or inline
        ):
            try:
                (
                ...
                if inline_assets:
                    inline_asset = List(
                        [inline_asset for inline_asset in inline_assets.values()],
                        separator=",",
                    )
        ...
        if font_sheet is None or asset is None:
            ...
            def glyphs(f=lambda x: x):
                for file_, storage in f(files):
                    ...
                    if m:
                        ...
                        height = None
                    _glyph = tempfile.NamedTemporaryFile(
            ...
            for type_, url in urls.items():
                ...
                if inline:
                    assets[type_] = inline_assets[type_] = List(
                        [Url.unquoted(url), String.unquoted(format_)]
                    )
                else:
            ...
            if len(font_sheets) > MAX_FONT_SHEETS:
                for a in sorted(
                    font_sheets, key=lambda a: font_sheets[a]["*"], reverse=True
                )[KEEP_FONT_SHEETS:]:
                    del font_sheets[a]
                log.warning(
                    "Exceeded maximum number of font sheets (%s)" % MAX_FONT_SHEETS
                )
...
def glyphs(sheet, remove_suffix=False):
    ...
    return List(
        [
            ...
            for f in sorted(
                set(
                    f.rsplit("-", 1)[0] if remove_suffix else f
                    for f in font_sheet
                    if not f.startswith("*")
                )
            )
