...
class CompassExtension(Extension):
    ...
    def handle_import(self, name, compilation, rule):
        ...
        map_name = (
            os.path.normpath(os.path.dirname(name)).replace("\\", "_").replace("/", "_")
        )
        kwargs = {}
        ...
        def setdefault(var, val):
            ...
            if _var in rule.context:
                kwargs[var] = calculator.interpolate(
                    rule.context[_var], rule, self._library
                )
            else:
        ...
        names = tuple(
            os.path.splitext(os.path.basename(file))[0] for file, storage in files
        )
        for n in names:
