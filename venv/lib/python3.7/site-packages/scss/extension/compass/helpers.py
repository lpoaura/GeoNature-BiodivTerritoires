...
def compact(*args):
    ...
    return List([arg for arg in args if arg], use_comma=use_comma,)
...
_elements_of_type = {
    ...
    "table-row-group": sorted(
        _elements_of_type_table_row_group.replace(" ", "").split(",")
    ),
    "table-header-group": sorted(
        _elements_of_type_table_header_group.replace(" ", "").split(",")
    ),
    "table-footer-group": sorted(
        _elements_of_type_table_footer_group.replace(" ", "").split(",")
    ),
    "table-row": sorted(
        _elements_of_type_table_footer_group.replace(" ", "").split(",")
    ),
    ...
    if file and inline:
        ...
        if re.match(r"^([^?]+)[.](.*)([?].*)?$", path.value):
            font_type = String.unquoted(
                re.match(r"^([^?]+)[.](.*)([?].*)?$", path.value).groups()[1]
            ).value
        ...
        url = make_data_url((mime if "/" in mime else "font/%s" % mime), file.read())
        file.close()
    else:
