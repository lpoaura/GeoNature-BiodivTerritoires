...
def __color_stops(percentages, *args):
    ...
    stops = [
        _s / max_stops if _s and not _s.is_simple_unit("%") else _s for _s in stops
    ]
    ...
    for i, s in enumerate(stops + [1.0]):
        if s is None:
            ...
            if start is not None:
                stride = (final - init) / Number(
                    end - start + 1 + (1 if i < len(stops) else 0)
                )
...
def __grad_position(index, default, radial, color_stops):
    try:
        ...
        if (
            radial
            and not stops.is_simple_unit("px")
            and (index == 0 or index == -1 or index == len(color_stops) - 1)
        ):
            log.warn(
...
def radial_gradient(*args):
    ...
    if shape_and_size and rendered_position:
        args.append(
            List(
                [shape_and_size, String.unquoted("at"), rendered_position],
                use_comma=False,
            )
        )
    ...
    def to__owg():
        args = [
            ...
            if position_and_angle is not None
            else "center",
            "0",
            grad_point(*position_and_angle)
            if position_and_angle is not None
        ...
        ret = (
            "-webkit-gradient("
            + ", ".join(to_str(a) for a in args or [] if a is not None)
            + ")"
        )
        return String.unquoted(ret)
    ...
    def to__svg():
        return radial_svg_gradient(
            *(list(color_stops) + list(position_and_angle or [String("center")]))
        )
...
def linear_gradient(*args):
    ...
    def to__owg():
        ...
        ret = (
            "-webkit-gradient("
            + ", ".join(to_str(a) for a in args if a is not None)
            + ")"
        )
        return String.unquoted(ret)
...
def __color_stops_svg(color_stops):
    ret = "".join(
        '<stop offset="%s" stop-color="%s"/>' % (to_str(s), c) for s, c in color_stops
    )
    return ret
...
def _linear_svg(color_stops, x1, y1, x2, y2):
    gradient = (
        '<linearGradient id="grad" x1="%s" y1="%s" x2="%s" y2="%s">%s</linearGradient>'
        % (
            to_str(Number(x1)),
            to_str(Number(y1)),
            to_str(Number(x2)),
...
def __radial_svg(color_stops, cx, cy, r):
    gradient = (
        '<radialGradient id="grad" gradientUnits="userSpaceOnUse" cx="%s" cy="%s" r="%s">%s</radialGradient>'
        % (
            to_str(Number(cx)),
            to_str(Number(cy)),
            to_str(Number(r)),
