...
from .layouts import (
    PackedSpritesLayout,
    HorizontalSpritesLayout,
    VerticalSpritesLayout,
    DiagonalSpritesLayout,
)
...
def sprite_map(g, **kwargs):
        ...
        for _glob in globs:
            if ".." not in _glob:  # Protect against going to prohibited places...
                ...
                if _files:
                    ...
                    base_name = (
                        os.path.normpath(os.path.dirname(_glob))
                        .replace("\\", "_")
                        .replace("/", "_")
                    )
    ...
    if files is not None:
        ...
        cache_path = os.path.join(
            config.CACHE_ROOT or ASSETS_ROOT, asset_file + ".cache"
        )
        ...
        if sprite_map is None or asset is None:
            ...
            direction = String.unquoted(
                kwargs.get("direction", config.SPRTE_MAP_DIRECTION)
            ).value
            ...
            dst_colors = [
                list(Color(v).value[:3]) for v in List.from_maybe(dst_colors) if v
            ]
            ...
            names = tuple(
                os.path.splitext(os.path.basename(file_))[0] for file_, storage in files
            )
            ...
            for name in names:
                ...
                if _dst_colors is None:
                    ...
                    _dst_colors = [
                        ...
                        for v in List.from_maybe(_dst_colors)
                        if v
                    ]
                ...
                if _src_colors is None:
                    ...
                    _src_colors = [
                        tuple(Color(v).value[:3]) for v in List.from_maybe(_src_colors)
                    ]
            ...
            sizes = tuple(
                (collapse_x or i.size[0], collapse_y or i.size[1]) for i in images()
            )

            if direction == "horizontal":
                layout = HorizontalSpritesLayout(
                    sizes, all_paddings, position=all_positions
                )
            elif direction == "vertical":
                layout = VerticalSpritesLayout(
                    sizes, all_paddings, position=all_positions
                )
            ...
            new_image = Image.new(
                mode="RGBA", size=(layout.width, layout.height), color=(0, 0, 0, 0)
            )
            ...
            if inline:
                asset = inline_asset = List(
                    [String.unquoted(url), String.unquoted(repeat)]
                )
            else:
            ...
            pickle.dump(
                (now_time, file_asset, inline_asset, sprite_map, sizes), cache_tmp
            )
            cache_tmp.close()
            ...
            if len(sprite_maps) > MAX_SPRITE_MAPS:
                for a in sorted(
                    sprite_maps, key=lambda a: sprite_maps[a]["*"], reverse=True
                )[KEEP_SPRITE_MAPS:]:
                    del sprite_maps[a]
                log.warning(
                    "Exceeded maximum number of sprite maps (%s)" % MAX_SPRITE_MAPS
                )
...
def sprites(map, remove_suffix=False):
    ...
    return List(
        [
            ...
            for s in sorted(
                set(
                    s.rsplit("-", 1)[0] if remove_suffix else s
                    for s in sprite_map
                    if not s.startswith("*")
                )
            )
