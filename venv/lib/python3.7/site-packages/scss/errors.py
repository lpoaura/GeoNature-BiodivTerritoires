...
class SassBaseError(Exception):
    ...
    def format_file_and_line(self, rule):
        return "line {rule.lineno} of {rule.source_file.path}".format(rule=rule,)
    ...
    def __str__(self):
        return "{message}\n{sass_stack}".format(
            message=self.format_message(), sass_stack=self.format_sass_stack(),
        )
...
class SassSyntaxError(SassBaseError):
    ...
    def __str__(self):
        ...
        if not self.desired_tokens:
            ...
            expected = "but expected {0}".format("".join(self.desired_tokens))
        else:
        ...
        return "{after}: {found} {expected}".format(
            after=after, found=found, expected=expected,
        )
...
class SassError(SassBaseError):
    ...
    def format_original_error(self):
        ...
        return "".join((type(self.exc).__name__, ": ", six.text_type(self.exc), "\n",))
...
class SassParseError(SassError):
    ...
    def format_prefix(self):
        decorated_expr, line = add_error_marker(
            self.expression, self.expression_pos or -1
        )
        return """Error parsing expression at {1}:\n{0}\n""".format(
            decorated_expr, self.expression_pos
        )
