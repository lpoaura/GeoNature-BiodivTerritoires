        ...
        def warned(fn, *args, **kwargs):
            for m in check_defaults:
                if kwargs[m] != defaults[m]:
                    warnings.warn(messages[m], exc.SADeprecationWarning, stacklevel=3)
            for m in check_kw:
                if m in kwargs:
                    warnings.warn(messages[m], exc.SADeprecationWarning, stacklevel=3)
...
def _decorate_cls_with_warning(cls, constructor, wtype, message, docstring_header=None):
    ...
    setattr(
        cls, constructor, _decorate_with_warning(constructor_fn, wtype, message, None),
    )
