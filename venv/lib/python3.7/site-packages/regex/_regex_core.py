...
class StringSet(RegexBase):
    ...
    def _compile(self, reverse, fuzzy):
        ...
        if fuzzy:
            ...
            for string in choices:
                branches.append(
                    Sequence([Character(c, case_flags=case_flags) for c in string])
                )
            ...
            if len(branches) > 1:
                ...
                branch = branches[0]
            branch = branch.optimise(self.info, reverse).pack_characters(self.info)
            ...
            return [(self._opcode[case_flags, reverse], index, min_len, max_len)]
    ...
    def _flatten(self, s):
        ...
        if isinstance(s, Branch):
            ...
            if n > 1:
                seq[:n] = [
                    String([c.value for c in seq[:n]], case_flags=self.case_flags)
                ]
