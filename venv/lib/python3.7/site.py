        ...
        if prefix:
            if sys.platform in ("os2emx", "riscos"):
                ...
                sitedirs = [
                    ...
                    os.path.join(
                        prefix, "python{}.{}".format(*sys.version_info), "lib-dynload"
                    ),
                ]
...
def addusersitepackages(known_paths):
    ...
    if os.name == "nt":
        ...
        USER_SITE = os.path.join(
            USER_BASE, "Python{}{}".format(*sys.version_info), "site-packages"
        )
    else:
        if env_base:
            USER_BASE = env_base
...
def virtual_install_main_packages():
    ...
    if _is_pypy:
        ...
        paths = [
            os.path.join(
                sys.real_prefix, "lib", "python{}.{}".format(*sys.version_info)
            )
        ]
        hardcoded_relative_dirs = paths[:]  # for the special 'darwin' case below
        lib64_path = os.path.join(
            sys.real_prefix, "lib64", "python{}.{}".format(*sys.version_info)
        )
